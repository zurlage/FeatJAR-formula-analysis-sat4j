...
public class ComputeAtomicSetsSAT4J extends ASAT4JAnalysis.Solution<BooleanSolutionList> implements IRandomDependency { // todo: here, a BooleanAssignmentList would be better
    ...
    public Result<BooleanSolutionList> compute(DependencyList dependencyList, Progress progress) {
        ...
        if (model1 != null) {
            ...
            BooleanSolution.resetConflicts(model1Copy, model2);
            for (int i = 0; i < model1Copy.length; i++) {
                ...
                if (varX != 0) {
                    ...
                    if (Result.of(false).equals(hasSolution)) {
                        ...
                        solver.getAssignment().remove();
                        BooleanSolution.resetConflicts(model1Copy, solver.getInternalSolution());
                        solver.shuffleOrder(random);
                    }
                }
            }
            final int fixedSize = solver.getAssignment().size();
            result.add(new BooleanSolution(solver.getAssignment().toSolution().get(0, fixedSize)));
